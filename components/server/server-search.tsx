...
const ServerSearch = ({
  ...
  return (
    ...
    className="group border px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50">
    ...
    return (
        <CommandGroup key={label} heading={label}>
            {data.map(({ id, icon, name }) => (
                ...
                key={id}> 
                    <span>{icon}</span>
                    <span>{name}</span> {/* Render the name here */}
                </CommandItem>
            ))}
        </CommandGroup>
    );
